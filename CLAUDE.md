...
**Orchestrator** 에게 작업을 지시합니다.
 **Orchestrator**가 요청을 분석하고 **팀 에이전트**에 위임합니다.

---

## ⚠️ 필수 규칙 — 반드시 준수

**모든 작업 요청에 대해 직접 실행을 금지합니다.**

작업 요청을 받으면 반드시 아래 순서를 따릅니다:

1. `agents/orchestrator.md`를 읽는다.
2. `Agent` 툴로 Orchestrator 서브에이전트를 스폰한다.
3. Orchestrator가 Secretary/Developer 팀에 위임한다.

**위반 예시 (금지)**:
- 요청을 받자마자 파일을 읽거나 코드를 수정하는 행위
- Agent 툴 없이 직접 분석·실행하는 행위
- Orchestrator를 건너뛰고 specialist를 직접 호출하는 행위

**예외**: 단순 질문(코드 설명, 개념 질문)은 직접 답변 가능.
