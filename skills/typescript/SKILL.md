...
name: typescript
description: >
  TypeScript 타입 시스템 설계 원칙, 베스트 프랙티스, 코드 패턴 가이드.
  fe-dev·be-dev·worker가 TypeScript 코드 작성 시 참조한다.
---

# TypeScript Skill

TypeScript 코드 작성 시 따라야 할 타입 설계 원칙과 패턴 가이드입니다.
이 스킬을 로드한 에이전트는 아래 기준에 따라 TypeScript 코드를 작성합니다.
...
이 스킬을 로드한 에이전트는 다음 순서로 TypeScript 작업을 수행합니다.

1. **타입 설계 먼저**: 도메인을 표현하는 타입/인터페이스를 먼저 정의한다 (Type-first)
2. **구현**: `strict: true` 모드에서 컴파일 에러 없이 작성, 타입 가드로 런타임 안전성 확보
3. **검증**: `tsc --noEmit` → `eslint` 순서로 검증

---

## 준수 사항 (Constraints)

- **any 금지**: `any` 사용 절대 금지. 불가피한 경우 `unknown` 후 타입 가드로 좁히기
